...
class Calculator extends React.Component {
  ...
  handleKeyPress = (e) => {
    ...
    else if ((e.shiftKey && e.key === "8") || e.key === "*" || e.which === "106") {
  ...
  input = (e) => {
    ...
    if (this.state.input.length > 0) {
      switch(this.state.operations[this.state.operations.length - 1]) {
        case '+':
          this.setState(state => ({
            output: state.input[state.input.length - 1] + Number(state.currInput),
          }));
          break;
        case '-':
          this.setState(state => ({
  ...
  operate = (e) => {
    if (this.state.currInput === '') {
      if (/\+|-/.test(e.target.value) && this.state.input.length === 0) {
      ...
      else if (this.state.operations[this.state.operations.length - 1].match(/x|\//)
          && e.target.value === '-') {
        this.setState(state => ({
          currInput: e.target.value,
          formula: state.formula + e.target.value
      ...
      else if (this.state.operations[this.state.operations.length - 1].match(/-/) 
          && e.target.value === '-') {
        this.setState(state => ({
          operations: state.operations.slice(0, state.operations.length - 1).concat('+'),
          formula: state.formula + e.target.value
        }));
      }
        ...
        this.setState(state => ({
          formula: state.formula + e.target.value,
          operations: state.operations.slice(0, state.operations.length - 1).concat(e.target.value)
        }));
  ...
  render() {
    ...
    const numberPads = numberIDs.map((id, index) => {
      return <button 
        key={`${id}${index}`} className="numbers" type="button" 
        id={id} value={index} onClick={this.input}
      >{index}
      </button>
    });
    ...
    const operatorPads = operatorIDs.map((id, index) => {
      return <button 
        key={`${id}${index}`} id={id} className="operators" 
        value={operators[index]} onClick={this.operate}
      >{operators[index]}
      </button>;
    });
      ...
      <div id="container">
        {
          ...
          <div>
            ...
            <h3><em>Immediate Execution Logic</em></h3>
          </div>
        }
        
        <p><em>by Adeniran</em></p>
        <div id="calculator">
          <div id="screen">
            <input id="formula" type="text" value={formula} readOnly/>
            <input 
              id="display" type="text" 
              value={(display) ? output : currInput} readOnly
            />
          </div>
          {numberPads}
          {operatorPads}
          <button id="equals" value="=" onClick={this.answer}>=</button>
          <button id="decimal" value= "." onClick={this.decimalize}>.</button>
          <button id="delete" onClick={this.delete}>del</button>
          <button id="clear" title="clear memory" onClick={this.clearScreen}>AC</button>
        </div>
      </div>
    );
  }
